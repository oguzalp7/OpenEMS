...
const ChakraModal = ({ children, isClosed, contentButtons, actionButtons, onClose }) => {
              ...
              <Stack justifyContent="space-between">
                ...
                <Stack flexDir={'row'}>
                  {showContentButtons && contentButtons && contentButtons.map((button, index) => (
                    <Button w={'full'} key={index} colorScheme={button.colorScheme} onClick={() => handleChangeContent(button.newContent, button.label)}>
                      {button.label}
                    </Button>
                  ))}
                </Stack>
